...
class SuspendJob implements ShouldBeUnique, ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if ($this->service->status !== Service::STATUS_ACTIVE || $this->service->cancellation?->type === 'immediate') {
            return;
        }
        ...
        if (ProviderOperationLifecycleService::usesDeferredOperations($this->service)) {
            ProviderOperationLifecycleService::complete($this->service, 'stop', $this->sendNotification, is_array($data) ? $data : []);
